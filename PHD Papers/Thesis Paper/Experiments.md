...
在多代理无线网络中，作者提出容错区块链共识协议，提出基于通信证明的容错共识算法，通过基于无线通信竞争方案和投票机制在区块链系统中达成共识。
...
测试协议运行一个时期的情况：
* 测试一个时期中交易处理的吞吐量，计算公式为 $Throughput = \frac{|txp_t|}{i\times 100\mu s + j \times 50\mu s}$，其中 $t = i + j $是一个时期的轮数， $i, j$ 是每个时期中两个阶段的轮数， $50\mu s$ 是 IEEE 802.11 设置的单位时隙长度。
...
仿真测试PoC共识算法的性能：当网络大小和其他参数发生变化时，在无线区块链网络中实现 $k$ 次 $\sigma-$共识所用的轮数。

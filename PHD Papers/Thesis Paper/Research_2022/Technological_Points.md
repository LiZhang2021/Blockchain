...
DAG区块链中，在一个由 $C$ 个移动无线网络节点组成的委员会中，能够容忍 $f = \lfloor\frac{C-1}{3}\rfloor$ 个节点发生拜占庭故障，能够保证达成共识的节点数量为 $Quorum = \lceil\frac{C+f+1}{2}\rceil$。
见证委员会首领负责将见证交易作为提案发送给委员会其他成员。
  ...
  * **更新区块链**：一旦接收到最终签名，节点将交易链接到本地链上，更新区块链。
...
针对DAG区块链中交易单元的确认，可以根据见证交易单元来构建系统主链确定稳定点。主链中稳定点之前的交易单元是完全相同的，因此可以为主链上的交易单元分配主链号，即稳定点之前的所有交易可以被确定全序。对于冲突交易则只承认主链号最早的交易单元的合法性，解决了交易双花的问题。

见证交易单元只能由见证委员会发出，并且每个见证交易单元必须引用前一个见证交易单元。见证交易单元确保DAG区块链将沿着一个方向增长。见证委员会的选举也是根据节点的稳定度来选举的。在无线DAG区块链系统中，网络节点的稳定度的具体定义如下：记$T_v$ 为无线网络节点 $v$ 在区块链系统中的活跃时间，记 $r_v = \frac{N_v}{N_{all}}$ 为无线节点在最近两个稳定点之间的共识比，其中 $N_v, N_{all}$ 分别是两个稳定点之间节点 $v$ 交易的数量和总数量。定义无线网络节点的稳定度为 
$$S_v= \alpha\times T_v+ \beta\times r_v$$
其中，权重系数 $\alpha, \beta$ 可根据偏好设置。在区块链系统运行初期，只有创世交易单元是稳定点时记节点的共识比 $r_v = 0$，此时节点的稳定度主要受节点的活动时间的影响。

随后可以根据节点的稳定度选举见证委员会，根据委员会成员之间的欧式距离或者位置等选举委员会首领生成见证交易，在委员会中运行一致性协议达成一致性并链接到DAG区块链上。
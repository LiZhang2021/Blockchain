...
在一个由 $C$ 个移动无线网络节点组成的委员会中，能够容忍 $f = \lfloor\frac{C-1}{3}\rfloor$ 个节点发生拜占庭故障，能够保证达成共识的节点数量为 $Quorum = \lceil\frac{C+f+1}{2}\rceil$。
...
见证交易单元只能由见证委员会发出，并且每个见证交易单元必须引用前一个见证交易单元。见证交易单元确保DAG区块链将沿着一个方向增长。见证委员会的选举也是根据节点的稳定度来选举的。在无线DAG区块链系统中，网络节点的稳定度的具体定义如下：

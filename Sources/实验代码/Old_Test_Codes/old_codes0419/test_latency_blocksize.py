...
class Network:
    ...
    # 事件处理
    def handle_event(self, curr_time, block_threshold, alpha, signs_threshold, max_transactions, consensus_begin_time, num_transactions):
        # 确定当前是否有首领节点
    ...
    def run(self, max_transactions, consensus_begin_time, consensus_end_time, num_transactions):
...
if __name__== '__main__':
    # block_size = [0, 100, 1000, 2000, 3000, 4000]
    block_size = numpy.arange(100, 2100, 100)
    latency = []
    throughput = []
    # MAX_TRANSACTIONS_SET = [200, 2000, 4000, 6000, 8000]
    MAX_TRANSACTIONS_SET = numpy.arange(200, 4200, 200)
    for MAX_TRANSACTIONS in MAX_TRANSACTIONS_SET:
        N1 = Network()
        CONSENSUS_BEGIN_TIME = []
        CONSENSUS_END_TIME = []
        NUM_TRANSACTIONS = []
        N1.run(MAX_TRANSACTIONS, CONSENSUS_BEGIN_TIME, CONSENSUS_END_TIME, NUM_TRANSACTIONS)
        ...
        print("共识时延", CONSENSUS_AVERAGE)
        print("交易数量", TRANSACTION_AVERAGE)
        latency.append(round(CONSENSUS_AVERAGE,4))
    print("区块大小", block_size)
    print("时延数据", latency)
    ...
    # 绘制折线图像1, 标签，线宽
    ax1.plot(block_size, latency, c='orangered', label='Latency', linewidth = 1) 
    mp.legend(loc=2)
    ax2 = ax1.twinx() # 创建第二个坐标轴
    ax2.plot(block_size, throughput, c='blue', label='Throughput', linewidth = 1) #同上, 'o-'
    mp.legend(loc=4)
    plt.grid(True)  # 样式风格：网格型
    # ax1.set_title("Double Y axis",size=22)  # 大标题
    ax1.set_xlabel('Block Size',size=18)  

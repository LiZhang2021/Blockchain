...
if __name__== '__main__':
    ...
    for gamma in gammas:
        gamma = round(gamma,2)
        print("诚实节点时间窗口发送占比", str(gamma))
        file_begin_time = open("Jamming_Begin_time.txt","a")
        file_begin_time.writelines(["Jamming_percentage\t", str(gamma), "\n"])
        file_begin_time.close()
        file_end_time = open("Jamming_End_time.txt","a")
        file_end_time.writelines(["Jamming_percentage\t", str(gamma), "\n"])
        file_end_time.close()
        ...
        while N1.current_time < MAX_SIMULATIOND_TIME and cblocks < 10:
            ...
            N1.Jamming_trans_random(SLOT, TRANSMISSION_RATE,gamma, TIME_WINDOW)

...
\setcounter{algorithm}{2}
    ...
    \State Compute elected probability of all nodes according to stability;
    \State Get sorted node list based on public key value;
...
\begin{breakablealgorithm}
	...
	\renewcommand{\algorithmicrequire}{\textbf{Input:}}
	\renewcommand{\algorithmicensure}{\textbf{Output:}}
	\caption{broadcsatSignature Subroutine}
	\label{alg1}
	\begin{algorithmic}[1]
        \For {Any node $v$ in $\{1, \cdots, N\}$}
            ...
            \If {$count_v > \hat{T}_v$}
                \State {$count_v = 1$}
                \If {$e_v == 0$} \textcolor{cyan}{//** No idle round in the past $\hat{T}_{v}$ slots}
                    \State {$\hat{p}_v = p_v/(1+\frac{1}{T_v})$}
                    \State {$\hat{T}_v = \hat{T}_v + 2$}
                \ElsIf {$e_v >= 1$}
                    \State {$\hat{p}_v = \hat{p}_v*(1+\frac{e_{v}}{T_{v}})$}

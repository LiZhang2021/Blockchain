...
* **轮盘赌抽签**
    ...
    $$(value, proof)=Sortition(sk, Block||Signature_{group})$$
    若 $\frac{value}{2^{bits(value)}}$ 落在某个节点所属的区间之内，则该节点被选举为出块节点。

* **验证抽签结果**
    根据随机可验证函数输出的值和证明，其他节点也可以验证出块节点选举结果的合法性。
    $$result=VerifySortition(pk, value, proof, Block, Signature_{group})$$
    如果验证结果为 $result= 1$，则出块节点的验证成功，该节点是合法的；如果结果为 $result= 0$，则出块节点的合法性将不被承认。

节点发现自己在当前轮被选中为合法的出块节点后，该节点会将近期的交易排序并生成区块 $B_v^{new}$，并广播区块消息 $m_B = (B_v^{new}, \sigma_v)$。其中 $\sigma_v$ 是节点的签名份额，其他节点在验证了区块和首领的有效性成功之后，也会广播签名份额给其他节点。<font color = red>（关于区块的生成过程可以再绘制一张图）</font>

**区块的验证和确认**

基于稳定性的共识协议主要

首领选举阶段、每个节点会根据当前的状态输入最新的区块哈希作为种子，查看自己是否是出块节点。

区块提出阶段，当节点确定了自己是出块节点之后
区块验证阶段、
链更新阶段、
区块链系统中共识算法的性质

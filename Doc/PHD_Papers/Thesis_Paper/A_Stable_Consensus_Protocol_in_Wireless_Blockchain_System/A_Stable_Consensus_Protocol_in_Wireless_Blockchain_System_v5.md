...
随着无线通信技术和区块链技术的高速发展，许多研究倾向于将区块链技术用于无线应用，如移动边缘计算、智能5G、车联网以及一些其他的物联网应用。区块链技术可以在分布式环境下提供可信赖和安全的资源共享服务，受到了学术界和工业界的高度关注。区块链的关键特性，如去中心化、持久性和可追溯性，为克服无线网络应用中存在的安全和信任问题提供了新的解决方案，特别是当它与智能合约技术相结合时。将区块链技术融入无线网络可以在无线环境中提供安全、可信、高效的资源共享、数据交互、安全接入控制、数据溯源和身份认证等服务。

目前，无线网络相关的区块链研究都直接将互联网中经典的区块链协议应用到无线网络环境中。这些区块链协议使用的共识算法通常依赖大量的资源消耗（比如，工作量证明[]）、复杂的设计（比如，权益证明[]）或者可靠的通信(比如PBFT[])。尽管这些区块链协议在互联网上运行良好，但它们不适用于资源有限且信道不稳定的无线网络。无线网络的开放通信受到环境的严重影响，信道带宽不稳定、易受干扰攻击是无线网络通信的瓶颈。这些障碍限制了传统区块链共识算法与无线网络相结合的应用，这也是研究无线网络上区块链协议的动机。

当前，有一些研究工作是利用无线网络的特性来设计区块链共识协议，使之更加适用于无线网络环境。文献[1]设计了一种基于工作量证明的稳定感知共识协议，该协议适用于具有高动态性的移动自组织网络。根据无线网络的通信特点，文献[2,3,4]中，作者都提出了适用于无线网络的共识算法。此外，考虑到无线信道易受干扰的特点，文献[7]的作者提出基于无线信道证明的适用于单跳无线网络的区块链共识协议， 该协议可以有效的抵抗SINR干扰模型下的阻塞攻击。文献[8]中，作者通过构建无线网络通信骨架的方式提出了适用于多跳无线网络的快速容错区块链共识协议。文献[9]利用无线通信协议信道争用机制，提出能够抵御女巫攻击的基于协作的拜占庭共识协议，适用于建立在开放无线网络中实时性要求高的物联网应用。这些共识协议的共识过程都需要以交互式的方式协商，出块节点需要承担比较重要的作用。如果在共识过程中出块节点发生故障，最终将导致共识失败影响共识协议的性能。

在本文中，为了解决上述无线区块链协议面临的问题，我们提出了一个适用于单跳无线网络的稳定区块链共识协议。我们设计的协议式一个类权益证明共识协议。采用随机选择方案和门限签名方案实现共识。在该协议中，根据由剩余活动时间和生成区块数量定义的节点稳定度随机、非交互地选举单个出块节点。所有节点只知道自身是否为出块节点，可以独立验证真实出块节点的合法性。这个设计可以降低出块节点选举阶段的算力消耗和通信消耗，同时也能够以较大概率选举出优质节点担任出块节点，提高系统共识过程的稳定性和处理交易效率。此外，我们采用门限BLS签名方案对出块节点和区块验证协商过程解耦。区块确认终止过程可以由系统中所有正确的共识节点在 $O(\log n)$ 内获得足够的投票来实现，并不单独依赖出块节点发出公告。这个设计可以提高共识过程的鲁棒性，降低由于节点故障或者链路不稳定导致无法达成共识的风险。

我们的主要贡献总结如下：
* 我们提出一种融合随机可验证函数和阈值BLS签名方案的稳定共识协议，可以在不稳定的无线网络环境中稳定地生成区块并达成共识。
* 我们通过节点的生存期和生成的节点块的数量来定义节点的稳定性。我们确定了权重系数，并分析了稳定性的敏感性。
* 根据节点的稳定性采取随机可验证的方式选举优质出块节点生成区块；采用门限BLS签名机制解耦出块节点和共识协商过程，提高协议的鲁棒性。
* 区块最终确定过程只能通过一轮部分签名交换来完成。因此，我们共识协议的强一致性可以有效避免区块链分叉。
* 当对手控制的投票权不足50%时，我们的协议保证了正确节点的持久性和活跃性。
* 最终，我们通过大量的仿真研究验证我们的理论分析。

本文剩余部分组成如下。下一节介绍区块链共识协议、无线共识算法和门限BLS签名方案等相关工作。第三节提出我们的模型和假设。第四节详细介绍稳定共识协议的细节。第五节讨论了协议的安全性和性能。第六节会给出共识协议相关的仿真结果。最后，在第七节给出了本文的结论。

## 2. Related Work

### 2.1 区块链共识协议

我们将当前的区块链共识协议可以分为基于资源证明的共识协议和基于协商的共识协议。更详细全面的区块链分类的综述可详见[10]。

基于资源证明的共识协议中参与共识的节点通过物理资源（比如，算力，存储等）或虚拟资源(比如，股份和名誉等）证明方式来竞争获取每一轮的出块权限。其中，最典型的基于物理资源证明的共识算法是比特币和以太坊使用的工作量证明(Proof of Work)。参与共识的节点通过消耗的计算资源证明来获得出块权限。但是PoW共识算法并不支持即时共识最终性[5]，而是在敌手的算力小于总算力 $50\%$ 时通过多个区块确认来保证概率一致性。因此，采用这类共识算法的区块链协议区块确认时延比较大，处理交易的数量比较有限。此外，基于物理资源证明的共识算法还有Burstcoin项目[13]中的空间证明 (Proof of Sapce, PoSpace)，共识参与者通过占用的内存或磁盘空间来竞争出块权限；以及燃烧证明（Proof of Burn），共识参与节点通过烧毁另一种“货币”，比如比特币，来获得生成区块的权限。物理资源证明共识算法需要通过消耗物理资源来获得生成区块的权限，这个过程会导致大量资源的浪费，对环境并不友好。为了降低共识的成本，提出了使用虚拟资源证明共识算法代替物理资源证明共识算法。典型的虚拟资源证明的共识算法是Peercoin 项目[11]中的权益证明(Proof of Stake, PoS)，参与共识的节点根据所持有的股份作为选举成为出块节点的权限证明，股份越多的节点，成为出块节点的概率就越大。以太坊中的Casper[6]是混合PoW和PoS的共识协议，旨在用PoS取代PoW。此外，虚拟资源证明的共识算法有 Parity 项目[12]中的权威证明(Proof of Authority, PoA)，参与共识的节点通过完成一个身份认证过程获得出块权限；GoChain项目[14]中的信誉证明(Proof of Reputation,PoR)[15]，共识参与节点需要拥有足够重要的信誉才能获取出块权限。

基于协商的区块链共识协议中，所有参与共识的节点通过执行本地计算和广播消息与其他节点通信协商对提出的区块达成共识。采用这种共识算法的区块链协议可以容忍拜占庭故障，还能提高协议的性能。典型的协商共识算法是Fabric v0.6.0[16]中实现的拜占庭容错算法(Practical Byzantine Fault Tolerant, PBFT)。该共识算法从全网节点中选出出块节点负责创建区块，通过与其他节点投票协商对区块达成全网共识。实际拜占庭容错协议无权益抵押或者资源的消耗会降低恶意节点的作恶成本，但是通过节点协作机制可以排除恶意行为对共识的影响。典型的协商共识算法还有NEO项目[17]中提出DBFT算法，授权的共识节点将获取出块权限，并通过投票协商对区块达成共识；在SBFT共识算法采用门限签名降低BFT共识的通信开销，但是仍然需要两轮通信完成签名聚合；Cosmos[19]中所采用的Tendermint共识算法实现了首领节点轮换机制和扩展交易传输；Algorand共识算法[20]采用了拜占庭一致性协议(Byzantine Agreement, BA)与随机可验证函数，对交易的集合达成共识。Ripple联盟链[18]项目的RPCA共识算法中通过一个信任节点列表对交易和区块投票协商达成一致。大部分协商类共识算法的共识协商过程依赖可信的首领节点，一旦首领节点出现故障将直接导致共识无法达成，只能通过切换视图来确保共识算法的活性。此外，协商类共识算法通常需要可靠的消息传输模型，因此具有较高的通信时间复杂度。比如，拜占庭共识算法的时间复杂度为 $O(n^2)$，当节点数量增加时，共识时延会急剧提升，性能会迅速下降。
...
我们首先测试网络大小为500个节点时，不同区块大小（设为 $100KB\sim 5000KB$）下时延和吞吐量的变化情况。我们的实验测试了连续10次共识的情况，最终结果是取10次的平均值。
![](blocksize.png)

如上图所示，共识时延随着区块大小的增加而稳定增长，当区块大小达到 ${\rm 1MB}$ 时共识时延小于 $0.5s$。有天域带宽的限制，当区块比较大时，所需要的传输时延也就比较大，最终导致共识所需的时延也会变大。平均吞吐量随着区块大小的增加而增加，最后趋于平缓。当区块大小增加时，区块中能够包含的交易数量会增加。随着共识时延的增加，虽然打包交易的数量会增加，但是吞吐量的增加会趋于平缓。因此，即使区块加大，我们协议能够保证在较低的时延时拥有较高的吞吐量。


其次，在固定块大小为 1MB时，我们实验分析共识协议中节点的数量对于共识时延的影响。测试不同节点数量（设置为$10\sim 1000$）下时延和吞吐量的变化情况，10次测量取平均值。
![](num_nodes.png)

随着节点数量的增加共识时延也增加的同时，平均吞吐量下降。但是在节点数量小于500时，共识时延增加几乎是线性增加，但是共识时延的值变化是比较小的。平均吞吐量的下降的趋势也是接近线性的。
...
我们实验分析了在固定区块大小为 $1MB$ 节点数量为 $500$ 时，共识时延和吞吐量随着带宽变化的情况。我们测试了不同带宽大小($10Mbps\sim 100Mbps)时连续10次共识协议运行情况。
![](bandwidth.png)

从上图可知，共识时延随着带宽的增加呈现对数下降，到最后趋于平缓。平均吞吐量随着带宽的增加而线性增加，最后趋于平缓。因此，当带宽越大，共识时延越低，吞吐量越高。这说明带宽会影响共识协议的性能，带宽越大协议处理交易的效率就越高。当带宽足够大时，协议处理交易的效率会达到最高从而趋于平缓。
...
敌手可以控制恶意节点竞争成为出块，但是拒绝生成有效区块。在我们的协议中，当前轮最终会生成一个空块，并且直接降低该节点的活动时间作为惩罚。在我们的设置中，协议并不依赖出块节点确保协议的安全性。因此，就算节点发起女巫攻击，也并不会影响我们协议的活性。

但是，女巫攻击的影响会导致这一轮生成一个空块，而所有诚实的节点会对空块达成共识。生成空块会影响协议的平均吞吐量。

女巫攻击中，攻击者控制的节点会竞争成为出块节点但并不打包区块或者并不打包一个有效区块。在我们的协议中，最终会生成并确认一个空块。当恶意节点没有成为出块节点时，在共识过程中将不对验证有效的区块进行签名确认。在之前的分析中，我们已经说明了女巫节点并不会影响协议的活性。

![](Sybil_Attack.png)

上图可知，共识时延并不会随着女巫节点的百分比的增加而明显提升，但是吞吐量会随着女巫节点数量的增加而缓慢，按下降。这是因为随着攻击者投入越多，女巫节点百分比的增加，当选为出块节点的概率会增加。生成空块的次数会增加，从而导致处理交易的数量会降低。但是并不会影响协议的活性。

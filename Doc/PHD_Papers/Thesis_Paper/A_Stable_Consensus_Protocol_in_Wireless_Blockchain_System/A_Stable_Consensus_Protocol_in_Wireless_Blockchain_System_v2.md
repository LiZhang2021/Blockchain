...
在我们的无线区块链网络中，传输失败可能会引起系统生成空块，从而增加网络的系统开销，因此，在进行分析之前，需要根据节点成熟成功的概率计算生成有效区块和空块的概率。

在协议的共识过程中，当节点在区块验证阶段没有收到超过 $f = \lfloor\frac{N+1}{2}\rfloor$ 个区块的签名份额时，就会触发生成空块。节点接收到至少 $f$ 个区块的签名份额的概率为 $P_{rs} = \sum_{k = f}^{N-1}C_{N-1}^kP_s^k(1 - P_s)^{N-1-k}$，协议生成有效区块的概率为 
$$P_{valid} = P\{L \geq f+1\} = \sum_{L=f}^{N-1}C_{N-1}^L P_{rs}^L(1 - P_{rs})^{N-1-L}$$
其中 $L$ 为成功接收到 $f$ 以上个区块签名份额节点的数量。因此，生成空块的的概率为 
$$P_{null} = 1 - P_{valid}。$$

#### 算力开销分析

我们的共识算法算力的消耗是对区块的验证，这个算力只占区块生成时间很小的一部分。生成空块时验证空块的时间也非常短可以直接忽略。因此，我们的协议中对于算力的消耗主要考虑协议运行时对区块中交易的验证，记为 $T_c$。记节点的算力为 $r_1$，当生成空块的概率 $0\leq P_{null} < 1$时，生成一个有效区块所需的哈希次数为
...
我们的共识算法主要是广播时延 $T_b$ 作为通信开销，区块验证时间 $T_c$ 也可以作为考虑的一部分。假设生成一个有效区块时，无节点故障时的通信次数为 $C_{normal}$；出块节点发生故障时的通信次数为 $C_{fault}$。假设生成一个有效区块之前发生了连续 $m$ 次出块节点故障。
  $$C_{PBFT} = \left\{
      \begin{aligned}
      & \sum_{m=0}^\infty P_{null}^mP_{valid}(mC_{fault} + C_{normal}), \text{if } P_v\neq 1, \\
      & \infty, \text{ if } P_v = 1.
      \end{aligned}
      \right.$$
  ...
  * 出块节点故障时：当出块节点故障时，则在 $T_1$ 时间内生成一个无效区块，在 $T_2$ 时间内生成一个空块。这段时间内的交易到达数为 $\lambda T_2$，增加的通信数是 $\lambda T_2(N-1)$，确认空块的通信数量为 $C_{fault} = \lambda T_2(N-1) + N(N-1)$。
最后计算得到在PBFT中平均通信数量为
  $$C_{Stable} = \left\{
      \begin{aligned}
      &, \text{if } P_v\neq 1, \\

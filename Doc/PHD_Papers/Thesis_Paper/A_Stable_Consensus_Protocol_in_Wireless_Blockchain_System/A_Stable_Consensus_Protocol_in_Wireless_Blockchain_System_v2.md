...
系统发生网络分区时，如果分区中节点数量都小于 $\lceil\frac{N-1}(2)\rceil$，则此时系统将进入分区等待。如果存在一个网络分区中节点数量超过 $\lceil\frac{N-1}(2)\rceil$ 有可能会生成一个有效块或者生成空块。
当首领节点在大分区中时，会生成一个有效区块。

* 在分区时生成有效区块的概率：
* 在分区时生成空块的概率：

为了防止由于网络分区导致频繁生成空块，需要一个分区检测的机制。

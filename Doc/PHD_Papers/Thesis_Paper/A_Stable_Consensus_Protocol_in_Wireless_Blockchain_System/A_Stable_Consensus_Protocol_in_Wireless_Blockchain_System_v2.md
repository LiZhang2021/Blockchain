...
在协议的共识过程中，当节点在区块验证阶段没有收到超过 $f = \lfloor\frac{N+1}{2}\rfloor$ 个区块的签名份额时，就无法生成区块的最终签名，会触发生成空块。节点接收到至少 $f$ 个区块的签名份额的概率为 $P_{rs} = \sum_{k = f}^{N-1}C_{N-1}^kP_s^k(1 - P_s)^{N-1-k}$，协议生成有效区块的概率为 
$$P_{valid} = P\{L \geq f+1\} = \sum_{L=f}^{N-1}C_{N-1}^L P_{rs}^L(1 - P_{rs})^{N-1-L}$$
其中 $L$ 为成功接收到 $f$ 以上个区块签名份额节点的数量，当 $L>f$ 时说明系统中大部分节点都能将有效区块添加到本地区块链上。因此，生成空块的的概率为 
$$P_{null} = 1 - P_{valid}。$$

#### 算力开销分析

记协议中每个节点的算力为 $r_1$，我们的共识算法算力的消耗主要是对区块的验证，其中对区块哈希验证的算力只占区块生成时间很小的一部分。因此，我们的协议中对于算力的消耗主要考虑协议运行时对区块中交易的验证，记验证交易的时间为 $T_c$。生成空块时验证空块的时间也非常短可以直接忽略。当生成空块的概率 $0\leq P_{null} < 1$时，生成一个有效区块所需的哈希次数为
$$H_{Stable} = \sum_{m=0}^{\infty}P_{null}^mP_{valid}(m+1)Nr_1T_c ,$$
...
我们的共识算法主要是广播时延作为通信开销，$T_b$ 是广播一次的时延，区块验证时间 $T_c$ 也可以作为考虑的一部分。协议正常生成一个有效区块需要两次广播：一次是首领节点广播区块，一次是节点广播区块的签名份额，因此时延为 $T_1 = 2T_b$。假设生成一个有效区块时，无节点故障时的通信次数为 $C_{normal}$；出块节点发生故障时的通信次数为 $C_{fault}$。假设生成一个有效区块之前发生了连续 $m$ 次出块节点故障。
  $$C_{Stable} = \left\{
      \begin{aligned}
      & \sum_{m=0}^\infty P_{null}^m\cdot P_{valid}\cdot (mC_{fault} + C_{normal}), \text{if } P_{null}\neq 1, \\
      & \infty, \text{ if } P_{null} = 1.
      \end{aligned}
      \right.$$
  * 无节点故障：协议中所有节点都是诚实的且无故障发生时，在 $T_1$ 时间内生成一个有效区块，假设交易到达速率为 $\lambda$，则交易到达数量为 $\lambda T_1$。计算得到无节点故障时的通信次数为 $C_{normal} = \lambda T_1(N-1) + (N-1) + N(N-1)$；
  * 节点故障时：当节点故障时，出块节点在 $T_1$ 时间内生成一个区块，并且由于网络问题在区块验证阶段失败。所有节点会在 $T_2$ 时间内生成一个空块，并签名确认该空块。这段时间内的交易到达数为 $\lambda T_2$，增加的通信数是 $\lambda T_2(N-1)$，确认空块的通信数量为 $C_{null} = \lambda T_2(N-1) + N(N-1)$，因此，发生故障时的通信次数为 $C_{fault} = C_{normal} + C_{null}$。
最后计算得到在我们的协议中平均通信数量为
  $$C_{Stable} = \left\{
      \begin{aligned}
      & \frac{P_{null}}{1 - P_{null}}\cdot C_{fault} + C_{normal}, \text{if } P_{null}\neq 1, \\
      & \infty, \text{ if } P_{null} = 1.
      \end{aligned}
      \right.$$

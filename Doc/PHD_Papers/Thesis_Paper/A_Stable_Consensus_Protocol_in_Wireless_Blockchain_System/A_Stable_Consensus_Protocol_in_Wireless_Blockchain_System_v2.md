...
我们的共识算法主要是广播时延 $T_b$ 作为通信开销，区块验证时间 $T_c$ 也可以作为考虑的一部分。假设生成一个有效区块时，无节点故障时的通信次数为 $C_{normal}$；出块节点发生故障时的通信次数为 $C_{fault}$。假设生成一个有效区块之前发生了连续 $m$ 次出块节点故障。
  $$C_{Stable} = \left\{
      \begin{aligned}
  ...
  * 出块节点故障时：当出块节点故障时，则在 $T_1$ 时间内生成一个区块，并且由于网络问题在区块验证阶段失败。所有节点会在 $T_2$ 时间内生成一个空块，并签名确认该空块。这段时间内的交易到达数为 $\lambda T_2$，增加的通信数是 $\lambda T_2(N-1)$，确认空块的通信数量为 $C_{fault} = C_{normal} + \lambda T_2(N-1) + N(N-1)$。
最后计算得到在PBFT中平均通信数量为
  $$C_{Stable} = \left\{
      \begin{aligned}
      &, \text{if } P_v\neq 1, \\

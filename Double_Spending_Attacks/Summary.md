...
对于链式结构的区块链，通过将两个具有相同输入的交易打包到区块中，但是最终攻击交易被确认到区块链上实现双花攻击成功。对于这类区块链的双花攻击模型有**一般的双花攻击模型?**和**自适应的双花攻击模型?**。下面分别给出两种攻击模型的分析和计算。
...
1. 系统模型假设
    ...
    * 假设节点在**固定时期?**长度中生成区块，一个时期 $T$ 内只能生成一个区块；
...
1. 问题描述
  ...
  *  假设诚实链长度为 $n$，欺诈链长度为 $m$，诚实链分支领先 $z = n - m$ 个区块的长度**插入一个图示**；
